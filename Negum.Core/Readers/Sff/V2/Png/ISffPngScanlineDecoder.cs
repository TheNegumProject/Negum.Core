...
namespace Negum.Core.Readers.Sff.V2.Png
{
    ...
    {
        public async Task<byte[]> DecodeAsync(byte[] scanline, byte[] previousScanline, int bytesPerPixel) =>
            scanline[0] switch
            {
                0 => scanline, // Skip filterType byte
                1 => this.DecodeSubFilter(scanline, bytesPerPixel),
                2 => this.DecodeUpFilter(scanline, previousScanline),
                3 => this.DecodeAverageFilter(scanline, previousScanline, bytesPerPixel),
                4 => this.DecodePaethFilter(scanline, previousScanline, bytesPerPixel),
                _ => throw new ArgumentException($"Unknown filter: {scanline[0]}")
            };
        ...
        protected virtual byte[] DecodePaethFilter(byte[] scanline, byte[] previousScanline, int bytesPerPixel)
        {
            ...
            for (; index < scanline.Length; ++index)
            {
                ...
                scanline[index] += this.GetPaethValue(left, above, aboveLeft);
            }
